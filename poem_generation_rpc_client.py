...
def create_rpc_client():
    ...
    return poem_generation_pb2_grpc.PoemGenerationStub(channel)

def send_rpc_request(client, genre, prefix):
    if prefix is None:
        prefix = ''
    response = client.Generate(poem_generation_pb2.GenRequest(genre=genre, prefix=prefix))
...
def generate():
    if request.method == 'GET':
        ...
        app.logger.info(msg)
        content = send_rpc_request(client, genre, prefix)
